...
/** \mainpage RoboComp::agentNavigation
 *
 * \section intro_sec Introduction
 *
 * The agentNavigation component...
 *
 ...
 *
 * Just: "${PATH_TO_BINARY}/agentNavigation --Ice.Config=${PATH_TO_CONFIG_FILE}"
 *
...
class agentNavigation : public RoboComp::Application
{
public:
	agentNavigation (QString prfx) { prefix = prfx.toStdString(); }
private:
	void initialize();
	std::string prefix;
	MapPrx mprx;
...
void ::agentNavigation::initialize()
{
...
int ::agentNavigation::run(int argc, char* argv[])
{
#ifdef USE_QTGUI
	...
	AriaNavigationPrx arianavigation_proxy;
	...
	try
	{
		if (not GenericMonitor::configGetString(communicator(), prefix, "AriaNavigationProxy", proxy, ""))
		{
			cout << "[" << PROGRAM_NAME << "]: Can't read configuration for proxy AriaNavigationProxy\n";
		}
		arianavigation_proxy = AriaNavigationPrx::uncheckedCast( communicator()->stringToProxy( proxy ) );
	}
	...
	rInfo("AriaNavigationProxy initialized Ok!");
	mprx["AriaNavigationProxy"] = (::IceProxy::Ice::Object*)(&arianavigation_proxy);//Remote server proxy creation example
	...
	try
	{
		...
		// Server adapter creation and publication
		if (not GenericMonitor::configGetString(communicator(), prefix, "AriaMapInformationTopic.Endpoints", tmp, ""))
		{
			cout << "[" << PROGRAM_NAME << "]: Can't read configuration for proxy AriaMapInformationProxy";
		}
		Ice::ObjectAdapterPtr AriaMapInformation_adapter = communicator()->createObjectAdapterWithEndpoints("ariamapinformation", tmp);
		AriaMapInformationPtr ariamapinformationI_ = new AriaMapInformationI(worker);
		Ice::ObjectPrx ariamapinformation = AriaMapInformation_adapter->addWithUUID(ariamapinformationI_)->ice_oneway();
		IceStorm::TopicPrx ariamapinformation_topic;
		if(!ariamapinformation_topic){
		try {
			ariamapinformation_topic = topicManager->create("AriaMapInformation");
		}
		catch (const IceStorm::TopicExists&) {
		//Another client created the topic
		try{
			ariamapinformation_topic = topicManager->retrieve("AriaMapInformation");
		}
		catch(const IceStorm::NoSuchTopic&)
		{
		...
		IceStorm::QoS qos;
		ariamapinformation_topic->subscribeAndGetPublisher(qos, ariamapinformation);
		}
		AriaMapInformation_adapter->activate();
